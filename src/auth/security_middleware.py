...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, max_requests: int = 500, time_window: int = 6000):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.url.path in ["/health", "/docs", "/redoc", "/openapi.json", "/token", "/auth/login", "/auth/register"]:
        ...
        if not is_local and self._is_rate_limited(client_ip):
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail="Rate limit exceeded. Please try again later."
            )
        elif is_local:
            # Still track but don't limit local requests as strictly
            current_time = time.time()
            client_requests = self.requests[client_ip]
            client_requests.append(current_time)
