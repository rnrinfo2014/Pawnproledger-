...
# Import API routers
from src.core.coa_api import router as coa_router
from src.core.daybook_api import router as daybook_router
from src.core.pledge_payment_api import router as pledge_payment_router
from src.core.receipt_api import router as receipt_router
...
# Pending pledge with detailed calculations
...
def create_customer(customer: CustomerCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_admin_user)):
    ...
    try:
        # 🆕 AUTO-CREATE COA ACCOUNT FOR CUSTOMER
        create_customer_coa_account(db, db_customer, customer.company_id)
        db.commit()
        db.refresh(db_customer)
        
        print(f"✅ Customer created with COA account: {db_customer.name} ({db_customer.acc_code})")
        return db_customer
        ...
        raise HTTPException(
            ...
            detail=f"Failed to create customer COA account: {str(e)}"
        )
...
def update_customer(customer_id: int, customer: CustomerCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_admin_user)):
    ...
    # Store old name for COA account update
    ...
    try:
        # 🆕 UPDATE COA ACCOUNT NAME IF CUSTOMER NAME CHANGED
        update_customer_coa_account(db, db_customer, old_name)
        db.commit()
        db.refresh(db_customer)
        
        if old_name != db_customer.name:
            print(f"✅ Customer updated with COA account: {db_customer.name} ({db_customer.acc_code})")
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to update customer COA account: {str(e)}"
        )
...
def delete_customer(customer_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_admin_user)):
    ...
    # Check if customer has active pledges
    active_pledges = db.query(PledgeModel).filter(
        PledgeModel.customer_id == customer_id,
        PledgeModel.status.in_(['active', 'overdue'])
    ).first()
    
    if active_pledges:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot delete customer with active pledges. Please settle all pledges first."
        )

    try:
        # 🆕 HANDLE COA ACCOUNT DELETION
        delete_customer_coa_account(db, db_customer)
        
        # Delete customer
        customer_name = db_customer.name
        customer_code = db_customer.acc_code
        db.delete(db_customer)
        db.commit()
        
        print(f"🗑️ Customer deleted: {customer_name} ({customer_code})")
...
# 🆕 Customer COA Balance and Management Endpoints

@app.get("/customers/{customer_id}/balance")
def get_customer_coa_balance(customer_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Get customer's current balance from their COA account"""
    try:
        balance_info = get_customer_balance(db, customer_id)
        return balance_info
...
@app.post("/customers/migrate-to-coa/{company_id}")
def migrate_customers_to_coa(company_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_admin_user)):
    ...
    try:
        ...
        results = migrate_existing_customers_to_coa(db, company_id)
        return {
            "message": "Customer COA migration completed",
            "results": results
        }
    except HTTPException:
        raise
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to migrate customers: {str(e)}"
        )

@app.get("/customers/{customer_id}/coa-info")
def get_customer_coa_info(customer_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Get customer's COA account information"""
    customer = db.query(CustomerModel).filter(CustomerModel.id == customer_id).first()
    if not customer:
        raise HTTPException(status_code=404, detail="Customer not found")
    ...
    return {
        ...
        "coa_account": {
            "account_id": coa_account.account_id,
            "account_name": coa_account.account_name,
            "account_code": coa_account.account_code,
            "account_type": coa_account.account_type,
            "is_active": coa_account.is_active
        } if coa_account else None
    }
...
def create_pledge(pledge: PledgeCreate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    ...
    try:
        ...
        # Get customer for accounting
        customer = db.query(CustomerModel).filter(CustomerModel.id == db_pledge.customer_id).first()
        
        # Create complete accounting entries for the pledge
        create_complete_pledge_accounting(
            db=db,
            pledge=db_pledge,
            customer=customer,
        ...
        db.rollback()
        print(f"HTTP Exception in create_pledge: {he.detail}")
        raise he
    except Exception as e:
        db.rollback()
        print(f"ERROR in create_pledge: {str(e)}")
        import traceback
        traceback.print_exc()
...
def read_pledges(
    ...
    query = db.query(PledgeModel).filter(PledgeModel.company_id == current_user.company_id)
    
    # Add search functionality
    if search:
        search_term = f"%{search}%"
        query = query.join(PledgeModel.customer).filter(
            or_(
                PledgeModel.pledge_no.ilike(search_term),
                PledgeModel.status.ilike(search_term),
                PledgeModel.remarks.ilike(search_term),
                CustomerModel.name.ilike(search_term),
                CustomerModel.phone.ilike(search_term)
            )
        )
    
    pledges = query.offset(skip).limit(limit).all()
    return pledges
...
def create_pledge_with_items(
    ...
    try:
        ...
        if customer:
            try:
                accounting_result = create_complete_pledge_accounting(db, db_pledge, customer, first_payment)
                print(f"✅ Accounting entries created for pledge {db_pledge.pledge_no}")
                print(f"   📊 Net customer liability: ₹{accounting_result['net_customer_liability']}")
            except Exception as e:
                print(f"❌ Failed to create accounting entries: {str(e)}")
                # Continue without failing the pledge creation
        ...
        db.rollback()
        print(f"ERROR in create_pledge_with_items: {str(e)}")
        import traceback
        traceback.print_exc()
...
@app.get("/customers/{customer_id}/pending-pledges", response_model=CustomerPendingPledgesResponse)
def get_customer_pending_pledges(
    ...
    for pledge, scheme in active_pledges:
        ...
        # Calculate payments from payment history
        total_paid = sum(payment.amount or 0.0 for payment in payments)
        
        # Calculate interest paid (subsequent payments only)
        subsequent_interest_paid = sum(
            payment.amount or 0.0 for payment in payments 
            if payment.payment_type == "interest"
        )
        
        # Total interest paid includes first month interest + subsequent payments
        interest_paid = first_month_interest + subsequent_interest_paid
        
        principal_paid = sum(
            payment.amount or 0.0 for payment in payments 
            if payment.payment_type in ["principal", "partial_principal"]
        )
        
        remaining_principal = total_loan - principal_paid
        remaining_interest = max(0.0, total_interest_due - interest_paid)
        ...
        # Current outstanding = remaining principal + remaining interest
        current_outstanding = remaining_principal + remaining_interest
...
@app.post("/customers/{customer_id}/multiple-pledge-payment", response_model=MultiPledgePaymentResponse)
...
def create_pledge_payment(
    ...
    try:
        db.add(db_payment)
        db.flush()  # Get the payment ID for accounting
        
        # Get customer information for accounting
        customer = db.query(CustomerModel).filter(
        ...
        try:
            ...
            print(f"💰 Payment accounting created: Voucher #{accounting_result['voucher_id']}, {accounting_result['entries_created']} entries")
        except Exception as accounting_error:
            print(f"❌ Error creating payment accounting: {str(accounting_error)}")
            # Continue with payment creation even if accounting fails
            # In production, you might want to rollback everything
            pass
...
from src.core.pledge_payment_api import router as pledge_payment_router
from src.core.receipt_api import router as receipt_router
from src.core.customer_ledger_api import router as customer_ledger_router
from src.core.payment_management_api import router as payment_management_router
from src.core.financial_year_api import router as financial_year_router

app.include_router(coa_router)
app.include_router(daybook_router)
app.include_router(pledge_payment_router)
...
@app.get("/dev/clear-rate-limit")
async def clear_rate_limit_dev(ip: str = None, current_user: UserModel = Depends(get_current_admin_user)):
    """
    Development endpoint to clear rate limit for specific IP
    Only works in development environment
    """
    if settings.environment != "development":
        raise HTTPException(status_code=404, detail="Not found")
    
    # Access the rate limit middleware
    for middleware in app.user_middleware:
        if hasattr(middleware, 'cls') and middleware.cls.__name__ == 'RateLimitMiddleware':
            rate_middleware = middleware.cls
            if hasattr(rate_middleware, 'requests'):
                if ip:
                    # Clear specific IP
                    if ip in rate_middleware.requests:
                        del rate_middleware.requests[ip]
                        return {"message": f"Rate limit cleared for IP: {ip}"}
                    else:
                        return {"message": f"No rate limit data found for IP: {ip}"}
                else:
                    # Clear all
                    rate_middleware.requests.clear()
                    return {"message": "All rate limits cleared"}
    
    return {"message": "Rate limit middleware not found"}
